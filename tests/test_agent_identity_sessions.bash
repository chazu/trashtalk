...
id=$(must @ Agent::Identity named: workspace-agent)
...
reject 'direct workspace duplicate rejected' @ Agent::Session openFor: "$id" archetype: "$arch" role: "$role" workspace: "$tmp/alias" profile: shell
check 'workspace lookup remains workspace-specific' "$s2" "$(@ Agent::Session currentFor: "$id" workspace: "$two")"
@ "$s1" pause >/dev/null
check 'paused current is reused' "$s1" "$(must ensure "$id" "$one")"
@ "$s1" close >/dev/null
check 'closed session excluded' '' "$(@ Agent::Session currentFor: "$id" workspace: "$one")"
s3=$(must ensure "$id" "$one")
@ "$s3" terminate >/dev/null
check 'terminated session excluded' '' "$(@ Agent::Session currentFor: "$id" workspace: "$one")"
...
reject 'direct open rejects duplicate identity session' @ Agent::Session openFor: "$gid" archetype: "$arch" role: "$role" workspace: "$one" profile: shell
...
mapfile -t run_info < <(@ Agent::Run startFor: "$g3" profile: shell)
reject 'fresh rejects active run' @ Gusgus fresh: "$two"
check 'active current remains open' open "$(@ "$g3" lifecycleState)"
reject 'second concurrent run rejected' @ Agent::Run startFor: "$g3" profile: shell
...
legacy=$(must @ Agent::Identity new)
@ "$legacy" handle: legacy-agent
@ "$legacy" owner: session-owner
@ "$legacy" save
old=$(must @ Agent::Session openFor: "$legacy" archetype: "$arch" role: "$role" workspace: "$one" profile: shell)
chosen=$(must @ Agent::Session openFor: "$legacy" archetype: "$arch" role: "$role" workspace: "$two" profile: shell)
...
delivery=$(must @ Agent::Delivery forSession: "$old" messages: "[\"$legacy_message\"]")
...
reject 'old conversation cannot start a run' @ Agent::Run startFor: "$old" profile: shell
reject 'historical session cannot receive live focus' @ Agent::Access liveSession: "$old"
check 'historical session remains inspectable' "$old" "$(must @ Agent::Access session: "$old")"
reject 'run-token caller cannot migrate' env TRASHTALK_RUN_TOKEN=foreign bash -c 'source "$1/lib/trash.bash"; @ Agent::Session select: "$2" forIdentity: "$3" owner: session-owner' fixture "$root" "$chosen" "$legacy"
...
cutover=$(must @ Agent::Identity new)
@ "$cutover" owner: session-owner
@ "$cutover" save
cutover_session=$(must @ Agent::Session openFor: "$cutover" archetype: "$arch" role: "$role" workspace: "$one" profile: shell)
_store_tx_before_commit() { @ "$cutover" selectCurrentSession: "$cutover_session" >/dev/null; }
reject 'migration fences a competing staged run start' @ Agent::Run startFor: "$cutover_session" profile: shell
...
bad=$(must @ Agent::Identity named: restricted)
strict=$(must @ Agent::Role define: strict-session revision: 1 capabilities: '[]' workspacePolicy: '["/allowed"]' runBudget: '{}')
before=$(@ Store countByClass: Agent::Session)
boxes=$(@ Store countByClass: Inbox)
reject 'denied ensure rolls back' @ Agent::Session ensureFor: "$bad" archetype: "$arch" role: "$strict" workspace: "$one" profile: shell
check 'no denied session leaked' "$before" "$(@ Store countByClass: Agent::Session)"
...
race=$(must @ Agent::Identity named: racing)
...
ids=$(@ Store idsOf: Agent::Session matching: "{\"identity\":\"$race\"}")
