...
# Agent::Identity, Agent::Archetype, Agent::Role, Agent::Session, Agent::Delivery, Agent::Run
...
# Inbox deliver: hands session:/agent: messages to Agent::Worker, which records a
...
# ==========================================
echo "1. Agent::Identity named: finds or creates, and creates its inbox"
# ==========================================
...
again=$(@ Agent::Identity named: 'gusgus')
...
assert_eq "findByHandle: finds it" "$identity" "$(@ Agent::Identity findByHandle: 'gusgus')"
assert_empty "findByHandle: misses unknown handle" "$(@ Agent::Identity findByHandle: 'nobody')"
...
bad=$(@ Agent::Identity named: 'bad handle;' 2>/dev/null)
rc=$?
...
# ==========================================
echo ""
echo "2. Agent::Archetype define: is idempotent and hashes instructions"
# ==========================================

instructions='You are a helpful assistant.'
archetype=$(@ Agent::Archetype define: 'assistant' revision: '1' instructions: "$instructions" profile: 'shell')
...
dup=$(@ Agent::Archetype define: 'assistant' revision: '1' instructions: 'different text' profile: 'other')
...
assert_eq "named:revision: finds it" "$archetype" "$(@ Agent::Archetype named: 'assistant' revision: '1')"
assert_empty "named:revision: misses other revision" "$(@ Agent::Archetype named: 'assistant' revision: '2')"
rev2=$(@ Agent::Archetype define: 'assistant' revision: '2' instructions: 'v2' profile: 'shell')
[[ "$rev2" != "$archetype" ]] && pass "new revision is a new record" || fail "new revision" "distinct" "same"

# ==========================================
echo ""
echo "3. Agent::Role define: idempotency, retryLimit, allowsWorkspace:"
# ==========================================

role=$(@ Agent::Role define: 'assistant' revision: '1' capabilities: '["inbox.read","message.send"]' workspacePolicy: '[]' runBudget: '{"turns":0,"usd":0,"retryLimit":3}')
assert_nonempty "role created" "$role"
assert_eq "define: returns existing role" "$role" "$(@ Agent::Role define: 'assistant' revision: '1' capabilities: '[]' workspacePolicy: '["/x"]' runBudget: '{}')"
assert_eq "named:revision: finds role" "$role" "$(@ Agent::Role named: 'assistant' revision: '1')"
...
strict=$(@ Agent::Role define: 'strict' revision: '1' capabilities: '[]' workspacePolicy: '["/opt/work","/srv/other"]' runBudget: '{}')
...
badrole=$(@ Agent::Role define: 'broken' revision: '1' capabilities: 'nope' workspacePolicy: '[]' runBudget: '{}' 2>/dev/null)
rc=$?
assert_eq "invalid policy JSON rejected" "1" "$rc"

# ==========================================
echo ""
echo "4. Agent::Session openFor: snapshots revisions and creates its inbox"
# ==========================================

session=$(@ Agent::Session openFor: $identity archetype: $archetype role: $role workspace: ${agent_workspace_root}/ws profile: 'shell')
...
denied=$(@ Agent::Session openFor: $identity archetype: $archetype role: $strict workspace: /home/x profile: 'shell' 2>/dev/null)
rc=$?
assert_eq "openFor: rejects disallowed workspace" "1" "$rc"
assert_empty "no session for disallowed workspace" "$denied"

found=$(@ Agent::Session findFor: $identity workspace: ${agent_workspace_root}/ws)
assert_eq "findFor: finds the open session" "$session" "$found"
assert_empty "findFor: misses other workspace" "$(@ Agent::Session findFor: $identity workspace: /elsewhere)"
...
# ==========================================
echo ""
echo "5. Agent::Session transition table"
# ==========================================
...
assert_eq "findFor: still sees reopened session" "$session" "$(@ Agent::Session findFor: $identity workspace: ${agent_workspace_root}/ws)"
...
assert_empty "findFor: excludes closed session" "$(@ Agent::Session findFor: $identity workspace: ${agent_workspace_root}/ws)"

# terminated from each live state, and terminal
term1=$(@ Agent::Session openFor: $identity archetype: $archetype role: $role workspace: ${agent_workspace_root}/t1 profile: 'shell')
assert_eq "open -> terminated" "terminated" "$(@ $term1 terminate)"
term2=$(@ Agent::Session openFor: $identity archetype: $archetype role: $role workspace: ${agent_workspace_root}/t2 profile: 'shell')
...
# ==========================================
echo ""
echo "6. Agent::Session workspaceFor:"
# ==========================================
...
assert_eq "git subdirectory resolves to repo root" "$expected_root" "$(@ Agent::Session workspaceFor: "$repo/a/b")"
plain=$(mktemp -d)
SCRATCH_DIRS+=("$plain")
mkdir -p "$plain/deep"
expected_plain=$(cd "$plain/deep" && pwd -P)
assert_eq "non-repo dir resolves to its real path" "$expected_plain" "$(@ Agent::Session workspaceFor: "$plain/deep")"
out=$(@ Agent::Session workspaceFor: "$plain/missing" 2>/dev/null); rc=$?
assert_eq "missing dir rejected rc" "1" "$rc"

# ==========================================
echo ""
echo "7. Agent::Delivery forSession: is idempotent by key"
# ==========================================

session=$(@ Agent::Session openFor: $identity archetype: $archetype role: $role workspace: ${agent_workspace_root}/ws profile: 'shell')
m1=$(@ Inbox send: 'please help' to: "session:$session" from: 'alice' subject: 'help' kind: 'question')
hooked=$(@ Agent::Delivery pendingFor: $session)
assert_eq "Inbox hook records one delivery for the message" "1" "$(line_count "$hooked")"
d1=$(@ Agent::Delivery forSession: $session messages: "[\"$m1\"]")
...
assert_eq "same messages return same delivery" "$d1" "$(@ Agent::Delivery forSession: $session messages: "[\"$m1\"]")"
assert_eq "forKey: finds it" "$d1" "$(@ Agent::Delivery forKey: "$expected_key")"
assert_empty "forKey: misses unknown key" "$(@ Agent::Delivery forKey: 'nope')"
assert_eq "messageIdList" "$m1" "$(@ $d1 messageIdList)"
assert_eq "senderOfFirstMessage" "alice" "$(@ $d1 senderOfFirstMessage)"
assert_eq "pendingFor: lists it" "$d1" "$(@ Agent::Delivery pendingFor: $session)"
...
# ==========================================
echo ""
echo "9. Agent::Delivery claim:run: is a one-shot compare-and-set"
# ==========================================

assert_eq "first claim succeeds" "true" "$(@ Agent::Delivery claim: $d1 run: $run)"
assert_eq "second claim fails" "false" "$(@ Agent::Delivery claim: $d1 run: $run)"
...
assert_eq "offeredFor: lists claimed delivery" "$d1" "$(@ Agent::Delivery offeredFor: $run)"
assert_eq "pendingFor: no longer lists it" "$d2" "$(@ Agent::Delivery pendingFor: $session)"

# ==========================================
echo ""
echo "10. Agent::Delivery transitions"
# ==========================================
...
assert_eq "re-claim after pending" "true" "$(@ Agent::Delivery claim: $d1 run: $run)"
...
d3=$(@ Agent::Delivery forSession: $session messages: "[\"$m1\",\"$m2\"]")
...
$d2" "$(@ Agent::Delivery pendingFor: $session)"

# ==========================================
echo ""
echo "11. Agent::Run current"
# ==========================================

unset TRASHTALK_RUN_TOKEN
out=$(@ Agent::Run current 2>/dev/null); rc=$?
...
# terminated session
tsession=$(@ Agent::Session openFor: $identity archetype: $archetype role: $role workspace: ${agent_workspace_root}/term profile: 'shell')
mapfile -t tlines < <(@ Agent::Run startFor: $tsession profile: 'shell')
trun="${tlines[0]}"
...
reply=$(@ Agent::Run result: 'here is the answer')
...
bob=$(@ Inbox named: 'bob')
q=$(@ Agent::Run askUser: 'which branch?')
...
assert_eq "blocked delivery still counts as held" "$d2" "$(@ Agent::Delivery offeredFor: $run)"
assert_eq "settle:note: processes a blocked delivery" "processed" "$(@ Agent::Run settle: $d2 note: 'answered')"
assert_eq "settle note stored" "answered" "$(@ $d2 note)"

# result: with no held delivery goes to the owner
fallback=$(@ Agent::Run result: 'nothing held')
...
n1=$(@ Agent::Run send: 'fyi' to: 'alice')
...
k1=$(@ Agent::Run send: 'progress 1' to: 'alice' key: 'progress')
k2=$(@ Agent::Run send: 'progress 2' to: 'alice' key: 'progress')
...
k3=$(@ Agent::Run send: 'other' to: 'alice' key: 'other')
