...
honker_bootstrap
sub=$(jq -c --arg d "$(@ WorkstationSchema digest)" '.adapterKind="command-receipt"|.streamName="workstation.command-receipts.v1"|.schemaDigest=$d|.filter={exitNot:0}|.debounceSeconds=3600' schemas/workstation/v1/fixtures/EventSubscription.valid.json)
@ EventSubscription createFrom: "$sub" >/dev/null
...
publish() { @ CommandReceipt publish: "$fixture" >/dev/null; }
...
@ WorkstationWorker tick >/dev/null || exit 1
...
publish; @ WorkstationWorker tick >/dev/null || exit 1; assert_count 4
...
@ CommandReceipt publish: "$(jq -c '.exitCode=0' <<<"$fixture")" >/dev/null
@ WorkstationWorker tick >/dev/null || exit 1
[[ $(@ "$consumer" offset) == 7 && $(count Attention) == 2 ]]
...
publish
if @ WorkstationWorker tick >/dev/null 2>&1; then echo 'FAIL: ack failure hidden'; exit 1; fi
[[ $(@ "$consumer" offset) == 7 ]]
snapshot=$(_db_sql "SELECT json_group_array(data) FROM instances WHERE class IN ('Attention','Message');")
eval "$saved_ack"
@ WorkstationWorker tick >/dev/null || exit 1
[[ $(@ "$consumer" offset) == 8 && $(_db_sql "SELECT json_group_array(data) FROM instances WHERE class IN ('Attention','Message');") == "$snapshot" ]]
...
publish
if @ WorkstationWorker tick >/dev/null 2>&1; then echo 'FAIL: projection failure hidden'; exit 1; fi
[[ $(@ "$consumer" offset) == 8 && $(_db_sql "SELECT json_group_array(data) FROM instances WHERE class IN ('Attention','Message');") == "$snapshot" ]]
eval "$saved_factory"
@ WorkstationWorker tick >/dev/null || exit 1
[[ $(@ "$consumer" offset) == 9 ]]
...
publish
if @ WorkstationWorker tick >"$TMPDIR/bad-record" 2>&1; then exit 1; fi
[[ $(@ "$consumer" offset) == 9 ]]
