...
delivery=$(@ Agent::Delivery offeredFor: "$run")
check 'fixture has active process' true "$(@ "$run" isProcessAlive)"
token=$(cat "$TOKEN_FILE")
check 'live run token resolves before termination' "$run" "$(TRASHTALK_RUN_TOKEN="$token" @ Agent::Run current)"

# A stop failure revokes authority but must not falsely finalize a live run.
__Agent__ShellDriver__class__interrupt_() { echo false; }
...
check 'terminated session revokes run token' '' "$(TRASHTALK_RUN_TOKEN="$token" @ Agent::Run current 2>/dev/null)"
unset -f __Agent__ShellDriver__class__interrupt_
...
@ Agent::Worker tickSession: "$session" >/dev/null
check 'terminated session cannot dispatch another run' 1 "$(@ Store countByClass: Agent::Run)"

# A reused PID must never receive the old run's signal.
other=$(@ Gusgus sessionFor: "$tmp")
mapfile -t started < <(@ Agent::Run startFor: "$other" profile: shell)
stale_run=${started[0]}
printf '%s\n' "$$" > "$tmp/stale.pid"
...
check 'stale PID needs no interrupt' true "$(@ Agent::Driver interrupt: "$stale_run")"
