...
# Old Agent::Session records on the user's store predate lifecycleState. Exercise
...
_db_sql "INSERT INTO instances(id,data) VALUES('$legacy',
  '{\"class\":\"Agent::Session\",\"_vars\":[\"sessionName\",\"sessionType\",\"context\",\"messageCount\"],\"sessionName\":null,\"sessionType\":null,\"context\":\"preserve legacy context\",\"messageCount\":0}');"
printf '%s\n' "terminate:$legacy" terminate '' > "$PICKS"
check 'browser dismisses after legacy termination' dismissed "$(@ Agent::Session browse)"
...
check 'fresh runtime retains terminated lifecycle' terminated "$(
    ...
    bash -c 'source "$1/lib/trash.bash"; @ Agent::Session findAll >/dev/null; @ "$2" lifecycleState' fixture "$root" "$legacy"
)"
