...
for state in pending pending blocked failed uncertain processed; do
    ...
    db_put "$id" "$(jq -cn --arg s "$session" --arg state "$state" '{class:"Agent::Delivery",session:$s,state:$state}')"
done
db_put agentdelivery_other "$(jq -cn --arg s "$other" '{class:"Agent::Delivery",session:$s,state:"pending"}')"
db_put agentrun_old "$(jq -cn --arg s "$session" '{class:"Agent::Run",session:$s,state:"failed",error:"earlier failure"}')"
db_put agentrun_latest "$(jq -cn --arg s "$session" '{class:"Agent::Run",session:$s,state:"running",error:""}')"
