...
honker_bootstrap
sub=$(jq -c --arg d "$(@ WorkstationSchema digest)" '.adapterKind="command-receipt"|.streamName="workstation.command-receipts.v1"|.schemaDigest=$d|.filter={exitNot:0}' schemas/workstation/v1/fixtures/EventSubscription.valid.json)
[[ $(@ EventSourceAdapter forKind: command-receipt) == CommandReceiptSourceAdapter ]]
...
envelope=$(@ CommandReceiptSourceAdapter normalize: "$record" for: "$sub")
...
[[ $(@ CommandReceiptSourceAdapter normalize: "$(jq -c '.payload|= (fromjson|.exitCode=0|tojson)' <<<"$record")" for: "$sub") == '' ]]
other=$(@ CommandReceiptSourceAdapter normalize: "$(jq -c '.payload|=(fromjson|.workspace="/other"|tojson)' <<<"$record")" for: "$sub")
[[ $(jq -r .groupKey <<<"$envelope") != $(jq -r .groupKey <<<"$other") ]]
[[ $(@ CommandReceiptSourceAdapter normalize: "$record" for: "$sub") == "$envelope" ]]
...
@ CommandReceipt publish: "$fixture"
[[ $(@ CommandReceiptSourceAdapter read: "$consumer" limit: 1 | jq length) == 1 ]]
for limit in 0 9 -1 '1;false'; do
 if @ CommandReceiptSourceAdapter read: "$consumer" limit: "$limit" >/dev/null 2>&1; then exit 1; fi
done
[[ $(@ "$consumer" offset) == 0 ]]
[[ $(@ Store countByClass: Attention) == 0 && $(@ Store countByClass: Message) == 0 && $(@ Store countByClass: AgentDelivery) == 0 ]]
