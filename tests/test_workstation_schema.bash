...
@ WorkstationSchema validate: "$valid" as: EventSubscription >/dev/null
...
@ WorkstationSchema validate: "$valid" as: EventSubscription >/dev/null
if @ WorkstationSchema validate: "$(jq -c .id=\"eventsubscription_other\" <<<"$valid")" as: EventSubscription >/dev/null 2>&1; then echo "Modified document skipped CUE"; exit 1; fi
if @ WorkstationSchema validate: "$valid" as: Attention >/dev/null 2>&1; then echo "Different root skipped CUE"; exit 1; fi
eval "$saved_vet"
invalid=$(cat schemas/workstation/v1/fixtures/EventSubscription.id.invalid.json)
if @ WorkstationSchema validate: "$invalid" as: EventSubscription >/dev/null 2>&1; then exit 1; fi
if @ WorkstationSchema validate: "$invalid" as: EventSubscription >/dev/null 2>&1; then echo "Failure was memoized"; exit 1; fi
