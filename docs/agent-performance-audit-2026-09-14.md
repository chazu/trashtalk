...
**Evidence (measured 1.73 s for a 21-assertion test with 0.82 s in the kernel; code in `lib/test-isolated.bash`):** every test file tars and untars `lib trash tests bin schemas Makefile`, runs `driver.bash fingerprint` (cat plus shasum of 250 KB), and copies the current-generation cache entries out of the 1,034-entry directory. With 90 runtime and 53 compiler files that is minutes of CPU per `make verify` spent before any test logic, and on eight jobs it sets a wall-clock floor near 30 s.
