...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Tools__Curl.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Tools__Curl.native --source")
		fmt.Fprintln(os.Stderr, "       Tools__Curl.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Curl" || receiver == "Tools::Curl" {
...
func loadInstance(db *sql.DB, id string) (*Curl, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Curl) error {
...
func createInstance(db *sql.DB, id string, instance *Curl) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Curl" || req.Instance == "Tools::Curl" {
...
func dispatch(c *Curl, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tools::Curl", nil
	case "id":
