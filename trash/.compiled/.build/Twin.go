...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Twin.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Twin.native --source")
		fmt.Fprintln(os.Stderr, "       Twin.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Twin" || receiver == "Twin" {
...
func loadInstance(db *sql.DB, id string) (*Twin, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Twin) error {
...
func createInstance(db *sql.DB, id string, instance *Twin) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Twin" || req.Instance == "Twin" {
...
func dispatch(c *Twin, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Twin", nil
	case "id":
