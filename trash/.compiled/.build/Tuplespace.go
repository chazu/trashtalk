...
func createInstance(db *sql.DB, id string, instance *Tuplespace) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func dispatch(c *Tuplespace, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tuplespace", nil
	case "id":
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("Tuplespace")
		instance := &Tuplespace{
			Class:     "Tuplespace",
