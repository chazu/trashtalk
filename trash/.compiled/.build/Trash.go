...
func createInstance(db *sql.DB, id string, instance *Trash) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func dispatch(c *Trash, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Trash", nil
	case "id":
