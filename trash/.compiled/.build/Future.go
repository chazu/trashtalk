...
type Future struct {
	...
	Pid         string   `json:"pid"`
	Result_file string   `json:"result_file"`
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Future.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Future.native --source")
		fmt.Fprintln(os.Stderr, "       Future.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Future" || receiver == "Future" {
...
func loadInstance(db *sql.DB, id string) (*Future, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Future) error {
...
func createInstance(db *sql.DB, id string, instance *Future) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Future" || req.Instance == "Future" {
...
func dispatch(c *Future, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Future", nil
	case "id":
