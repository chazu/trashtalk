...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Fidget.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Fidget.native --source")
		fmt.Fprintln(os.Stderr, "       Fidget.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Fidget" || receiver == "Fidget" {
...
func loadInstance(db *sql.DB, id string) (*Fidget, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Fidget) error {
...
func createInstance(db *sql.DB, id string, instance *Fidget) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Fidget" || req.Instance == "Fidget" {
...
func dispatch(c *Fidget, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Fidget", nil
	case "id":
...
func (c *Fidget) Example() string {
	sendMessage(Widget, "example")
	return " Hello from Fidget "
}
