...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Tools__Netcat.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Tools__Netcat.native --source")
		fmt.Fprintln(os.Stderr, "       Tools__Netcat.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Netcat" || receiver == "Tools::Netcat" {
...
func loadInstance(db *sql.DB, id string) (*Netcat, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Netcat) error {
...
func createInstance(db *sql.DB, id string, instance *Netcat) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Netcat" || req.Instance == "Tools::Netcat" {
...
func dispatch(c *Netcat, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tools::Netcat", nil
	case "id":
...
func InstallCommand() string {
	return "nc is typically pre-installed on Unix systems"
}
