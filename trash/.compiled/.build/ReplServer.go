...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: ReplServer.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       ReplServer.native --source")
		fmt.Fprintln(os.Stderr, "       ReplServer.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "ReplServer" || receiver == "ReplServer" {
...
func loadInstance(db *sql.DB, id string) (*ReplServer, error) {
	var data string
	err := db.QueryRow("SELECT data FROM instances WHERE id = ?", id).Scan(&data)
	if err != nil {
		return nil, err
	}
	var instance ReplServer
...
func saveInstance(db *sql.DB, id string, instance *ReplServer) error {
...
func createInstance(db *sql.DB, id string, instance *ReplServer) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "ReplServer" || req.Instance == "ReplServer" {
...
func dispatch(c *ReplServer, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "ReplServer", nil
	case "id":
	...
	case "socketPath_":
		if len(args) < 1 {
			return "", fmt.Errorf("socketPath_ requires 1 argument")
		}
		return c.SocketPath(args[0])
	case "clearHistory":
		c.ClearHistory()
		return "", nil
	default:
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("ReplServer")
		instance := &ReplServer{
			Class:      "ReplServer",
...
func (c *ReplServer) SocketPath(path string) (string, error) {
...
func (c *ReplServer) ClearHistory() {
	_ivar_set
	c.History
	"[]"
}
