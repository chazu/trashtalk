...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: File.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       File.native --source")
		fmt.Fprintln(os.Stderr, "       File.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "File" || receiver == "File" {
...
func createInstance(db *sql.DB, id string, instance *File) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "File" || req.Instance == "File" {
...
func dispatch(c *File, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "File", nil
	case "id":
