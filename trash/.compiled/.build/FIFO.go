...
type FIFO struct {
	...
	KeeperPid string   `json:"keeperPid"`
	WriterPid string   `json:"writerPid"`
	ReaderPid string   `json:"readerPid"`
}

func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: FIFO.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       FIFO.native --source")
		fmt.Fprintln(os.Stderr, "       FIFO.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "FIFO" || receiver == "FIFO" {
...
func loadInstance(db *sql.DB, id string) (*FIFO, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *FIFO) error {
...
func createInstance(db *sql.DB, id string, instance *FIFO) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "FIFO" || req.Instance == "FIFO" {
...
func dispatch(c *FIFO, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "FIFO", nil
	case "id":
...
func (c *FIFO) WriterPid() {
	_ivar
	c.WriterPid
}

func (c *FIFO) ReaderPid() {
	_ivar
	c.ReaderPid
}
