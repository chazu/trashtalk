...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Process.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Process.native --source")
		fmt.Fprintln(os.Stderr, "       Process.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Process" || receiver == "Process" {
...
func loadInstance(db *sql.DB, id string) (*Process, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Process) error {
...
func createInstance(db *sql.DB, id string, instance *Process) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Process" || req.Instance == "Process" {
...
func dispatch(c *Process, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Process", nil
	case "id":
