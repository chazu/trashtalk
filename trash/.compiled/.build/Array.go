...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Array.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Array.native --source")
		fmt.Fprintln(os.Stderr, "       Array.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Array" || receiver == "Array" {
...
func loadInstance(db *sql.DB, id string) (*Array, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Array) error {
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Array" || req.Instance == "Array" {
