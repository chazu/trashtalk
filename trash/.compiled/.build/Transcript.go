...
type Transcript struct {
	...
	OutputView        string   `json:"outputView"`
	InputField        string   `json:"inputField"`
	Layout            string   `json:"layout"`
	Running           string   `json:"running"`
	TranscriptHistory string   `json:"transcriptHistory"`
}

func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Transcript.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Transcript.native --source")
		fmt.Fprintln(os.Stderr, "       Transcript.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Transcript" || receiver == "Transcript" {
...
func loadInstance(db *sql.DB, id string) (*Transcript, error) {
	var data string
	err := db.QueryRow("SELECT data FROM instances WHERE id = ?", id).Scan(&data)
	if err != nil {
		return nil, err
	}
	var instance Transcript
...
func saveInstance(db *sql.DB, id string, instance *Transcript) error {
...
func createInstance(db *sql.DB, id string, instance *Transcript) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Transcript" || req.Instance == "Transcript" {
...
func dispatch(c *Transcript, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("Transcript")
		instance := &Transcript{
			Class:             "Transcript",
...
func (c *Transcript) SetSession(s string) (string, error) {
...
func (c *Transcript) Session() {
	_ivar
	c.Session
}

func (c *Transcript) Close() {
	_ivar_set
	c.Running
	"false"
}
