...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Protocol.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Protocol.native --source")
		fmt.Fprintln(os.Stderr, "       Protocol.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Protocol" || receiver == "Protocol" {
...
func loadInstance(db *sql.DB, id string) (*Protocol, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Protocol) error {
...
func createInstance(db *sql.DB, id string, instance *Protocol) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Protocol" || req.Instance == "Protocol" {
...
func dispatch(c *Protocol, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Protocol", nil
	case "id":
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("Protocol")
		instance := &Protocol{
			Class:     "Protocol",
