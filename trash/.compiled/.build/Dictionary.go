...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Dictionary.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Dictionary.native --source")
		fmt.Fprintln(os.Stderr, "       Dictionary.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Dictionary" || receiver == "Dictionary" {
...
func loadInstance(db *sql.DB, id string) (*Dictionary, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Dictionary) error {
...
func createInstance(db *sql.DB, id string, instance *Dictionary) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Dictionary" || req.Instance == "Dictionary" {
...
func dispatch(c *Dictionary, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
	...
	case "_getItemsObject":
		return c._getItemsObject(), nil
	case "at_":
		if len(args) < 1 {
	...
	case "includesKey_":
		if len(args) < 1 {
			return "", fmt.Errorf("includesKey_ requires 1 argument")
		}
		return c.IncludesKey(args[0])
	case "removeAt_":
		if len(args) < 1 {
			return "", fmt.Errorf("removeAt_ requires 1 argument")
		}
		return c.RemoveAt(args[0])
	case "size":
		return c.Size(), nil
	case "isEmpty":
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("Dictionary")
		instance := &Dictionary{
			Class:     "Dictionary",
...
func (c *Dictionary) GetItems() string {
	return c.Items
}

func (c *Dictionary) SetItems(newItems string) (string, error) {
	newItemsInt, err := strconv.Atoi(newItems)
	if err != nil {
		return "", err
	}
	_ = newItemsInt
...
func (c *Dictionary) _getItemsObject() string {
	return c.Items
}

func (c *Dictionary) At(key string) (string, error) {
	keyInt, err := strconv.Atoi(key)
	if err != nil {
		return "", err
	}
	_ = keyInt
	return _jsonObjectAt(c.Items, key), nil
}

func (c *Dictionary) At_put(key string, value string) (string, error) {
	keyInt, err := strconv.Atoi(key)
	if err != nil {
		return "", err
	}
	...
	_ = valueInt
	c.Items = _jsonObjectAtPut(c.Items, key, valueInt)
	return strconv.Itoa(valueInt), nil
}

func (c *Dictionary) IncludesKey(key string) (string, error) {
	keyInt, err := strconv.Atoi(key)
	if err != nil {
		return "", err
	}
	_ = keyInt
	return _boolToString(_jsonObjectHasKey(c.Items, key)), nil
}

func (c *Dictionary) RemoveAt(key string) (string, error) {
	keyInt, err := strconv.Atoi(key)
	if err != nil {
		return "", err
	}
	...
	oldValue = _jsonObjectAt(c.Items, key)
	c.Items = _jsonObjectRemoveKey(c.Items, key)
	return strconv.Itoa(oldValue), nil
}

func (c *Dictionary) Size() string {
	return strconv.Itoa(_jsonObjectLen(c.Items))
}

func (c *Dictionary) IsEmpty() string {
	return _boolToString(_jsonObjectIsEmpty(c.Items))
}

func (c *Dictionary) Clear() string {
	c.SetItems("{}")
	return strconv.Itoa(self)
}
