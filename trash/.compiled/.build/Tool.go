...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Tool.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Tool.native --source")
		fmt.Fprintln(os.Stderr, "       Tool.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Tool" || receiver == "Tool" {
...
func loadInstance(db *sql.DB, id string) (*Tool, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Tool) error {
...
func createInstance(db *sql.DB, id string, instance *Tool) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Tool" || req.Instance == "Tool" {
...
func dispatch(c *Tool, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tool", nil
	case "id":
