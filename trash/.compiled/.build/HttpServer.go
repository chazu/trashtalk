...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: HttpServer.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       HttpServer.native --source")
		fmt.Fprintln(os.Stderr, "       HttpServer.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "HttpServer" || receiver == "HttpServer" {
...
func loadInstance(db *sql.DB, id string) (*HttpServer, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *HttpServer) error {
...
func createInstance(db *sql.DB, id string, instance *HttpServer) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "HttpServer" || req.Instance == "HttpServer" {
...
func dispatch(c *HttpServer, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
	...
	case "root_":
		if len(args) < 1 {
			return "", fmt.Errorf("root_ requires 1 argument")
		}
		return c.Root(args[0])
	case "port_":
		if len(args) < 1 {
			return "", fmt.Errorf("port_ requires 1 argument")
		}
		return c.Port(args[0])
	case "stop":
		c.Stop()
		return "", nil
	default:
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("HttpServer")
		instance := &HttpServer{
			Class:     "HttpServer",
...
func (c *HttpServer) Root(path string) (string, error) {
...
func (c *HttpServer) Port(p string) (string, error) {
