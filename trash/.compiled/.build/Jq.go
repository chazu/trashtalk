...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Tools__Jq.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Tools__Jq.native --source")
		fmt.Fprintln(os.Stderr, "       Tools__Jq.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Jq" || receiver == "Tools::Jq" {
...
func loadInstance(db *sql.DB, id string) (*Jq, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Jq) error {
...
func createInstance(db *sql.DB, id string, instance *Jq) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Jq" || req.Instance == "Tools::Jq" {
...
func dispatch(c *Jq, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tools::Jq", nil
	case "id":
