...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Person.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Person.native --source")
		fmt.Fprintln(os.Stderr, "       Person.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Person" || receiver == "Person" {
...
func loadInstance(db *sql.DB, id string) (*Person, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Person) error {
...
func createInstance(db *sql.DB, id string, instance *Person) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Person" || req.Instance == "Person" {
...
func dispatch(c *Person, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Person", nil
	case "id":
