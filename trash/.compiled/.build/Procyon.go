...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Tools__Procyon.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Tools__Procyon.native --source")
		fmt.Fprintln(os.Stderr, "       Tools__Procyon.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Procyon" || receiver == "Tools::Procyon" {
...
func loadInstance(db *sql.DB, id string) (*Procyon, error) {
	var data string
	err := db.QueryRow("SELECT data FROM instances WHERE id = ?", id).Scan(&data)
	if err != nil {
		return nil, err
	}
	var instance Procyon
...
func saveInstance(db *sql.DB, id string, instance *Procyon) error {
...
func createInstance(db *sql.DB, id string, instance *Procyon) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Procyon" || req.Instance == "Tools::Procyon" {
...
func dispatch(c *Procyon, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Tools::Procyon", nil
	case "id":
...
func InstallCommand() string {
	return "go install github.com/chazu/procyon/cmd/procyon@v0.7.0"
}
