...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: IDE.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       IDE.native --source")
		fmt.Fprintln(os.Stderr, "       IDE.native --hash")
		os.Exit(1)
	}
...
func loadInstance(db *sql.DB, id string) (*IDE, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *IDE) error {
...
func createInstance(db *sql.DB, id string, instance *IDE) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "IDE" || req.Instance == "IDE" {
...
func dispatch(c *IDE, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "IDE", nil
	case "id":
...
func (c *IDE) SetSession(s string) (string, error) {
