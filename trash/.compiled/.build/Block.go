...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: Block.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       Block.native --source")
		fmt.Fprintln(os.Stderr, "       Block.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "Block" || receiver == "Block" {
...
func loadInstance(db *sql.DB, id string) (*Block, error) {
	var data string
...
func saveInstance(db *sql.DB, id string, instance *Block) error {
...
func createInstance(db *sql.DB, id string, instance *Block) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "Block" || req.Instance == "Block" {
...
func dispatch(c *Block, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "Block", nil
	case "id":
