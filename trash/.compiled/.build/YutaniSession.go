...
type YutaniSession struct {
	Class       string   `json:"class"`
	CreatedAt   string   `json:"created_at"`
	Vars        []string `json:"_vars"`
	SessionId   string   `json:"sessionId"`
	Host        string   `json:"host"`
	EventCoproc string   `json:"eventCoproc"`
}

func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: YutaniSession.native <instance_id> <selector> [args...]")
		fmt.Fprintln(os.Stderr, "       YutaniSession.native --source")
		fmt.Fprintln(os.Stderr, "       YutaniSession.native --hash")
		os.Exit(1)
	}
	...
	if receiver == "YutaniSession" || receiver == "YutaniSession" {
...
func loadInstance(db *sql.DB, id string) (*YutaniSession, error) {
	var data string
	err := db.QueryRow("SELECT data FROM instances WHERE id = ?", id).Scan(&data)
	if err != nil {
		return nil, err
	}
	var instance YutaniSession
...
func saveInstance(db *sql.DB, id string, instance *YutaniSession) error {
...
func createInstance(db *sql.DB, id string, instance *YutaniSession) error {
	data, err := json.Marshal(instance)
	if err != nil {
		return err
	}
...
func handleServeRequest(db *sql.DB, req *ServeRequest) ServeResponse {
	if req.Instance == "" || req.Instance == "YutaniSession" || req.Instance == "YutaniSession" {
...
func dispatch(c *YutaniSession, instanceID string, selector string, args []string) (string, error) {
	switch selector {
	case "class":
		return "YutaniSession", nil
	case "id":
	...
	case "setHost_":
		if len(args) < 1 {
			return "", fmt.Errorf("setHost_ requires 1 argument")
		}
		return c.SetHost(args[0])
	case "setSessionId_":
		if len(args) < 1 {
			return "", fmt.Errorf("setSessionId_ requires 1 argument")
		}
		return c.SetSessionId(args[0])
	case "sessionId":
		c.SessionId()
		return "", nil
	case "host":
...
func dispatchClass(selector string, args []string) (string, error) {
	switch selector {
	case "new":
		id := generateInstanceID("YutaniSession")
		instance := &YutaniSession{
			Class:       "YutaniSession",
...
func (c *YutaniSession) SetHost(h string) (string, error) {
...
func (c *YutaniSession) SetSessionId(sid string) (string, error) {
...
func (c *YutaniSession) SessionId() {
	_ivar
	c.SessionId
}

func (c *YutaniSession) Host() {
	_ivar
	c.Host
}

func (c *YutaniSession) Disconnect() {
	c.DestroySession()
	c.StopEventStream()
}
